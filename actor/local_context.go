...
func (ctx *localContext) ChildrenMap() map[string]*PID {
	r := make(map[string]*PID, ctx.children.Len())
	ctx.children.ForEach(func(_ int, p PID) {
		r[p.Id] = &p
	})
	return r
}
...
func (ctx *localContext) InvokeSystemMessage(message interface{}) {
	...
	default:
		logger.Error("unknown system message, msg=[%v]", msg)
	}
}
